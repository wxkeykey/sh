...
j2s(){
pro=`ps aux | awk '{print $1":"$2":"$8}'`

for i in $pro
do
   p1=`echo $i | awk -F: '{print $1}'`
   p2=`echo $i | awk -F: '{print $2}'`
   p3=`echo $i | awk -F: '{print $3}'`
  
   if [ "Z" == "$p3" ]
   then
      echo $p1 "---" $p2 "---" $p3
      kill -9 $p2
      echo "It is kill the zombie process $p2 who is belong the $p1....ok"
   else
      :
   fi
done
...
if ! pgrep nginx > /dev/null; then
    ...
    # 或者使用 systemctl 命令（取决于您的系统和Nginx的安装方式）
...
echo "进程统计信息如下"
echo "总进程数量为:$procs"
echo "运行 进程数为:$running"
echo "已停止 进程数为:$stoped"
echo "睡眠 进程数为:$sleeping"
echo "僵尸 进程数为:$zombie"
}
...
green "  8.查找僵尸进程"
green "  9.本机所有开放端口"
green "  10.主机Ping检测(需要本地ip.txt)"
green "  11.统计 Linux 进程相关数量信息"
green "  12.Nginx出现异常时自动重启"
green "  0.退出脚本 "
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
readp "请输入数字:" Input
case "$Input" in     
